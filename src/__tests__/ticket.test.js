...
const defaultRegex = require("../ticket").DEFAULT_TICKET_REGEX;

describe("grabTicket", () => {
  it("should return ticket id without colon", () => {
    const title = "ABC-123: New Feature";
    const result = grabTicket(title, defaultRegex);
...
describe("grabTicket by custom regex", () => {
  it("should return ticket id with colon", () => {
    ...
    const regex = new RegExp("^[A-Z,a-z]{2,}-\\d{1,}:", "g")
    const result = grabTicket(title, regex);
